package com.ychd.ycwwz.base_library.jpush;
...
public class TagAliasOperatorHelper {
            ...
            switch (msg.what){
                case DELAY_SEND_ACTION:
                    ...
                    if(msg.obj !=null && msg.obj instanceof  TagAliasBean){
                        TLog.INSTANCE.i(TAG,"on delay time");
                        sequence++;
                        ...
                        if(context!=null) {
                            ...
                            TLog.INSTANCE.i(TAG,"#unexcepted - context was null");
                        }
                    }else{
                        TLog.INSTANCE.i(TAG,"#unexcepted - msg obj was incorrect");
                    }
                    break;
                case DELAY_SET_MOBILE_NUMBER_ACTION:
                    if(msg.obj !=null && msg.obj instanceof String) {
                        TLog.INSTANCE.i(TAG, "retry set mobile number");
                        sequence++;
    ...
    public void handleAction(Context context, int sequence, String mobileNumber){
        ...
        TLog.INSTANCE.i(TAG,"sequence:"+sequence+",mobileNumber:"+mobileNumber);
    ...
    public void handleAction(Context context, int sequence, TagAliasBean tagAliasBean){
        ...
        if(tagAliasBean == null){
            TLog.INSTANCE.i(TAG,"tagAliasBean was null");
            return;
        }
        ...
        if(tagAliasBean.isAliasAction){
            switch (tagAliasBean.action){
                ...
                case ACTION_SET:
                    ...
                    break;
                default:
                    TLog.INSTANCE.i(TAG,"unsupport alias action type");
                    return;
            }
        }else {
            switch (tagAliasBean.action) {
                ...
                case ACTION_CLEAN:
                    ...
                    break;
                default:
                    TLog.INSTANCE.i(TAG,"unsupport tag action type");
                    return;
            }
        }
    }
    private boolean RetryActionIfNeeded(int errorCode,TagAliasBean tagAliasBean){
        if(!ExampleUtil.isConnected(context)){
            TLog.INSTANCE.i(TAG,"no network");
            return false;
        }
        //返回的错误码为6002 超时,6014 服务器繁忙,都建议延迟重试
        if(errorCode == 6002 || errorCode == 6014){
            TLog.INSTANCE.i(TAG,"need retry");
            if(tagAliasBean!=null){
    ...
    public void onTagOperatorResult(Context context, JPushMessage jPushMessage) {
        ...
        TLog.INSTANCE.i(TAG,"action - onTagOperatorResult, sequence:"+sequence+",tags:"+jPushMessage.getTags());
        TLog.INSTANCE.i(TAG,"tags size:"+jPushMessage.getTags().size());
        init(context);
        ...
        if(jPushMessage.getErrorCode() == 0){
            //设置tag成功记录一下
            AccessManager.Companion.setJiPushTagIsSuccess(true);
            TLog.INSTANCE.i(TAG,"action - modify tag Success,sequence:"+sequence);
            setActionCache.remove(sequence);
            String logs = getActionStr(tagAliasBean.action)+" tags success";
            TLog.INSTANCE.i(TAG,logs);
            //ExampleUtil.showToast(logs, context);
        }else{
    ...
    public void onCheckTagOperatorResult(Context context, JPushMessage jPushMessage){
        int sequence = jPushMessage.getSequence();
        TLog.INSTANCE.i(TAG,"action - onCheckTagOperatorResult, sequence:"+sequence+",checktag:"+jPushMessage.getCheckTag());
        init(context);
        //根据sequence从之前操作缓存中获取缓存记录
        TagAliasBean tagAliasBean = (TagAliasBean)setActionCache.get(sequence);
        if(tagAliasBean == null){
        ...
        if(jPushMessage.getErrorCode() == 0){
            TLog.INSTANCE.i(TAG,"tagBean:"+tagAliasBean);
            setActionCache.remove(sequence);
    ...
    public void onAliasOperatorResult(Context context, JPushMessage jPushMessage) {
        int sequence = jPushMessage.getSequence();
        TLog.INSTANCE.i(TAG,"action - onAliasOperatorResult, sequence:"+sequence+",alias:"+jPushMessage.getAlias());
        init(context);
        //根据sequence从之前操作缓存中获取缓存记录
        TagAliasBean tagAliasBean = (TagAliasBean)setActionCache.get(sequence);
        if(tagAliasBean == null){
        ...
        if(jPushMessage.getErrorCode() == 0){
            TLog.INSTANCE.i(TAG,"action - modify alias Success,sequence:"+sequence);
            setActionCache.remove(sequence);
    ...
    public void onMobileNumberOperatorResult(Context context, JPushMessage jPushMessage) {
        int sequence = jPushMessage.getSequence();
        TLog.INSTANCE.i(TAG,"action - onMobileNumberOperatorResult, sequence:"+sequence+",mobileNumber:"+jPushMessage.getMobileNumber());
        init(context);
        if(jPushMessage.getErrorCode() == 0){
            TLog.INSTANCE.i(TAG,"action - set mobile number Success,sequence:"+sequence);
            setActionCache.remove(sequence);
        }else{
